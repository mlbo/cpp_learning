...
本题复用并重连输入节点，不创建或释放业务节点；返回值只是新的头指针，节点所有权仍由调用者/测试夹具负责。前置条件是两条输入链都无环且各自非递减；若两链在合并前已经共享尾部，直接按普通模板重连可能重复接入同一节点，接口应禁止这种输入或单独处理。
...
3. **边界处理**：剩余节点直接连接
4. **可达性不变量**：改链前保存各自未处理头，已合并前缀有序且两个未处理后缀仍可达
