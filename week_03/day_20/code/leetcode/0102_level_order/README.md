...
> **文档职责**：BFS 为什么要“入队即标记”以及队列大小快照的通用原理，以 [Day 20 主文](../../../README.md#day20-bfs-enqueue-mark) 为准。本文只展开 LC 102 的树层边界与输出形式。
...
上表的空间是**不计返回结果的额外工作空间**：BFS 队列最多保存一层附近的节点，DFS 使用递归调用栈。返回的二维数组本身必须保存 n 个节点值，两种方法的输出空间都是 O(n)。
