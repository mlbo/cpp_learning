...
> **本题差异**：BFS 和“冻结当前层大小”已在 [LC 102](../0102_level_order/) 主讲，本题新增的只是**输出层的顺序**。因此阅读重点是“先得到自顶向下结果再反转”与“构造时头插”的取舍，不再把队列模板当成新知识。
...
这些仍是不计返回结果的额外工作空间；输出数组自身为 O(n)。BFS 后反转的时间是 O(层数)，不超过 O(n)；`deque` 方案避免最后反转，但还需要把层容器转成题目要求的 `vector`，不应根据一次小输入运行就声称它必然更快。
