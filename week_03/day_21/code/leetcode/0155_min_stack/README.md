...
> **文档职责**：栈适配器的空容器前置条件以 [Day 15](../../../../day_15/README.md#day15-stack-adaptor) 为准。本文专注“每个深度同步保存当前最小值”的状态不变量。
...
题面通常保证不对空栈调用 `pop/top/getMin`，但一个可复用类必须选择并写明空栈契约：抛 `std::underflow_error`、返回 `std::optional<int>`，或把“非空”作为显式前置条件。辅助栈方案的不变量是 `mainStack.size() == minStack.size()`，且同一深度的 `minStack.top()` 是主栈对应前缀的最小值；任何只弹一个栈的异常路径都会破坏该关系。差值法还要用足够宽的中间类型计算 `val - minVal`，避免有符号整数溢出带来未定义行为。
