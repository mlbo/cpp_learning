/**
 * @file item28_folding_rules.cpp
 * @brief EMC++ Item 28: 理解引用折叠规则
 * 
 * 引用折叠是实现完美转发的技术基础。
 * 
 * 核心规则：
...
void demonstrateAutoCollapse() {
    ...
    std::cout << "\nauto&& 从普通表达式推导时使用对应的引用折叠规则:\n";
    std::cout << "  直接大括号列表初始化是 auto 的 initializer_list 特例，不是转发引用\n";
    
    auto&& r1 = x;          // 表达式 x 是左值
    auto&& r2 = lr;         // lr 是左值引用
    auto&& r3 = 20;         // 字面量表达式 20 是纯右值
    auto&& r4 = std::move(x); // std::move 产生 xvalue
...
void run() {
    ...
    std::cout << "  4. auto&& 从普通表达式推导时遵循相同规则；大括号列表是例外\n";
}

} // namespace day24::item28_rules

void runItem28RulesDemo() {
    day24::item28_rules::run();
}
